...
public class CollaborativeFilterRecommender implements
		epnoi.recommeders.Recommender {
	...
	private PreferenceArray _getUserPreferences(User user) {
		...
		int numberOfRatedItems = model.getRatingsByUser(user.getURI(), Rating.WORKFLOW_RATING).size();
		...
		for (Rating rating : model.getRatingsByUser(user.getURI(), Rating.WORKFLOW_RATING)) {
	...
	//	System.out.println(user.getID() + "UserPreferences for "+ userPreferences);
